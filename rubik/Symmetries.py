...
# S_URF3,   120-градусный разворот куба вокруг оси между URF углом и DBL углом (не используем)
...
# Вычисляем 16 симметрий путем перебора базовых симметрий
# (S_F2)x2 * (S_U4)x3 * (S_LR2)x4
SYM_CUBIES = []
cubie = cc.CubieCube()
...
def create_conj_twist():
    conj_twist = [[0 for _ in range(SYM_N)] for _ in range(2187)]
...
def create_conj_ud_edges():
    conj_ud_edges = [[0 for _ in range(SYM_N)] for _ in range(40320)]
